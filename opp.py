...
# 메뉴를 숨기고 한국어 음성(TTS)을 지원하는 특수 장치
st.markdown("""
    <style>
        ...
        if os.path.exists(fname):
            try:
                with open(fname, "r", encoding="utf-8") as f:
                    ...
                    questions = []
                    for line in content:
                        # [에러 해결]: 역슬래시 문제를 일으키던 코드를 삭제하고 안전한 정규식만 사용
                        # 불필요한 태그와 양끝 공백을 제거합니다.
...
if st.session_state.step == "init":
    ...
    if not MATH_DB:
        st.error("학습 데이터(.txt) 파일을 찾을 수 없습니다. 6개 학기 파일이 깃허브 폴더에 있는지 확인해 주세요.")
        st.stop()
        
    st.session_state.sel_sem = st.selectbox("학기 선택", list(MATH_DB.keys()))
    
    # 선택된 학기 내의 소단원 리스트를 추출합니다.
    units = sorted(list(set([d["unit"] for d in MATH_DB[st.session_state.sel_sem]])))
    st.session_state.sel_unit = st.selectbox("오늘 테스트할 소단원", units)
    
    if st.button("테스트 시작"):
        # 해당 단원의 문제만 필터링 후 무작위로 섞습니다.
...
if prompt := st.chat_input("답변을 입력하세요 (그만하려면 '그만')"):
    ...
    [가장 중요한 상호작용 원칙]
    1. 로봇 같은 표현 금지 ("질문을 시작합니다", "다시 말할게요" 등 금지)
    2. 수식은 반드시 'x의 제곱', '2분의 1', '루트 3'처럼 한글로만 말할 것
    3. 정답이면 크게 칭찬하고 다음 질문(Q{st.session_state.q_idx}. 형식)을 던질 것
    4. 틀리면 바로 정답을 주지 말고 힌트를 최대 2번 주어 스스로 답하게 유도할 것
